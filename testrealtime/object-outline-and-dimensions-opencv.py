...
cv2.createTrackbar("threshold", windowName, 75, 255, nothing)#255
...
while(showLive):
    ...
    # define range of blck color in HSV
    lower_bl = np.array([0, 0, 0])
    upper_bl = np.array([360, 100, 10])

    # Threshold the HSV image to get only black colors
    mask = cv2.inRange(hsv, lower_bl, upper_bl)

    ret,thresh1 = cv2.threshold(hsv,thresh,255,cv2.THRESH_BINARY)
    ...
    kernel = np.ones((kern,kern),np.uint8) # square image kernel used for erosion [1 1 1 1....;;;;;111111]
    
    itera=cv2.getTrackbarPos("iterations", windowName)
    # dilation =   cv2.dilate(thresh1, kernel, iterations=itera)
    # erosion = cv2.erode(dilation,kernel,iterations = itera) # refines all edges in the binary image smoothiong the contour  using the above kernel
    #
    ...
    contours,hierarchy = cv2.findContours(mask,cv2.RETR_TREE,cv2.CHAIN_APPROX_NONE) # find contours with simple approximation cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE




    # closing = cv2.cvtColor(closing,cv2.COLOR_GRAY2RGB)
    cv2.drawContours(mask, contours, -1, (128,255,0), 1)
    ...
    for contour in contours:
        ...
        if shape is "square" or "rectangle":
